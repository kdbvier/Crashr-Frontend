...
export const getExactTokenSymbolValue = (symbol: string, value: number) => {
    const _value = value > 1000000 ? (Math.floor(value / 1000000) + 'm') : (value > 1000 ? (Math.floor(value / 1000) + 'k') : value)
    if (symbol === "ADA") {
        return `₳${_value}`
    } else if (symbol === "Mad Dog Token") {
        return `${_value}MD`
    } else {
...
export const getVolumeExpression = (volume: number) => {
    ...
    if (m_value >= 1) {
        return Math.floor(m_value) + "m";
    } else {
        const t_value = volume / 1000;
        if (t_value >= 1) {
            return Math.floor(t_value) + "k";
        } else {
...
export const getExactTokenAmount = (amount: number, tokenName: string) => {
    if (!POLICYID_ASSET_LIST.hasOwnProperty(tokenName)) {
        ...
        if (POLICYID_ASSET_LIST[tokenName].decimals === 0) {
            ...
            return Math.floor(amount / Math.pow(10, POLICYID_ASSET_LIST[tokenName].decimals))
        }
    }
}
...
export const getNFTDetailData = async (assetID: string) => {
    const { data } = await axios.get("https://fk6vsmvml8.execute-api.eu-west-2.amazonaws.com/default/getNFTinfo?unit=" + assetID);
    if (data) {
        return {
            image: getExactImageFormat(data.onchain_metadata.image),
        }
    }
}

export const getPassedTime = (timestamp: number) => {
    ...
    if (diff > 0) {
        ...
        if (day > 1) return `${Math.floor(day)} days ago`
            ...
            if (hour > 1) return `${Math.floor(hour)} hrs ago`
            else {
                let min = diff / 60 / 1000;
                return `${Math.floor(min + 1)} mins ago`
            }
        }
    }
}

export const getActivityTag = (input: string) => {
    const activities: any = {
        "cancel-nft": "cancel.listing",
        "list-nft": "list.nft",
        "edit-nft": "edit.nft",
        "buy-nft": "buy.nft",
        "migrate-nft": "migrate.nft",
        "buy-cart": "buy.cart",
    }

    return activities[input] ? activities[input] : input;
}