...
const Polls = () => {
  ...
  const { myBalance, votingData, colorMode } = useGlobalContext()
  ...
  useEffect(() => {
    if (votingData && Object.values(votingData).length > 0) {
      ...
      let filtered_arr2: any = filter_arr1.filter(item => item.name.toLowerCase().includes(search.toLocaleLowerCase()));
      ...
      <Container paddingTop='12px' smMarginTop='-30px'>
        ...
        <FlexBox marginTop='28px' flexWrap='wrap' gap='16px 80px' justifyContent='start'>
          ...
          {activeShowVotingData &&
            ...
            activeShowVotingData.map((item: any, index: number) => {
              // @ts-ignore
