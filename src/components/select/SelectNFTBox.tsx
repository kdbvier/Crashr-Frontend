...
const SelectNFTBox = (
  ...
  useEffect(() => {
    ...
    let filtered_arr = Object.values(TOKEN_ARRAY).filter((item: any) => item.value.toLowerCase().includes(searchInput.toLowerCase()));
  ...
  return (
    <SelectNFTBoxStyle>
      ...
      >
        {
          formData.selectedNFTs.length > 0 ? formData.selectedNFTs.length + ' NFT(s) Chosen' : 'Select NFT’s from wallet'
        }
        {
          showOptions
            ?
