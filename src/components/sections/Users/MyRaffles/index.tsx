...
const MyRaffles = () => {
  ...
  useEffect(() => {
    if (myRaffleData.length > 0) {
      ...
      // @ts-ignore
      let filtered_arr: any = filteredArr1.filter(item => item.name.toLowerCase().includes(search.toLocaleLowerCase()));
    ...
    <>
      <FlexBox marginBottom='32px' smDirection='row' gap="20px" justifyContent='start' direction='column'>
        ...
        <FlexBox flexWrap='wrap' gap='70px 90px'>
          {activeShowingData && activeShowingData.length > 0 && floorPriceList && activeShowingData.map((item: any, index: number) => {
